...
class PermissionUser {
  ...
  hasPermission(permission) {
    ...
    if(this.hasGlobalPermission(permission))return true;
  ...
  hasGlobalPermission(permission) {
    if(config.get("owner") == this.user.id) return true;

    if(!filesystem.existsSync(`global-permissions.json`))filesystem.writeFileSync(`global-permissions.json`, "{}", 'utf-8');;
    var permissions = JSON.parse(filesystem.readFileSync(`global-permissions.json`, "utf8"));
  ...
  addGlobalPermission(permission) {
    ...
    filesystem.writeFileSync(`global-permissions.json`, JSON.stringify(permissions), 'utf-8');
  }


  removeGlobalPermission(permission) {
    if(!filesystem.existsSync(`global-permissions.json`))filesystem.writeFileSync(`global-permissions.json`, "{}", 'utf-8');;
    var permissions = JSON.parse(filesystem.readFileSync(`global-permissions.json`, "utf8"));
